...
# api key
my_vworld_key <- "33261B4D-3A52-3E89-BB6D-F3395E8DE218"
my_domain <-  "https://www.vworld.kr/dev/v4dv_opn2dmap2guide_s001.do"
# 3. 위경도 좌표를 넣으면 행정동명을 찾아주는 함수 정의
get_administrative_dong <- function(lng, lat) {
  # [교정] &location= 을 필수 파라미터인 &point= 로 수정했습니다!
  ...
  tryCatch({
    ...
    return(NA) # 시스템 에러 발생 시 프로그램이 멈추지 않게 NA 반환
  })
}
